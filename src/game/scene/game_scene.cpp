...
namespace game::scene
{
    ...
    void GameScene::createUnitsPortraitUI()
    {
        ...
        auto padding = ui_config_->getUnitPanelPadding(); // 单位肖像框之间的间隔
    ...
    void GameScene::arrangeUnitsPortraitUI(engine::ui::UIElement *anchor_panel, const glm::vec2 &frame_size, float padding)
    {
        ...
        // 更新panel的size
        anchor_panel->setSize(glm::vec2(padding + anchor_panel->getChildren().size() * (frame_size.x + padding), padding * 2 + frame_size.y));
    }
