...
namespace engine::resource
{
    ...
    SDL_Texture *TextureManager::loadTexture(std::string_view file_path)
    {
        ...
        if (!raw_texture)
        {
            spdlog::error("[TextureManager] 加载纹理失败：'{}':{}", file_path, SDL_GetError());
            return nullptr;
        }
        textures_.emplace(file_path, std::unique_ptr<SDL_Texture, SDLTextureDeleter>(raw_texture));
        spdlog::debug("[TextureManager] 成功载入并缓存纹理：{}", file_path);
    ...
    SDL_Texture *TextureManager::getTexture(std::string_view file_path)
    {
        ...
        spdlog::warn("[TextureManager] 未缓存纹理'{}'，尝试加载它", file_path);
    ...
    void TextureManager::unloadTexture(std::string_view file_path)
    {
        ...
        if (it != textures_.end())
        {
            spdlog::debug("[TextureManager] 卸载纹理：{}", file_path);
            textures_.erase(it); // 借助删除器删除
        }
        else
        {
            spdlog::warn("[TextureManager] 尝试删除不存在的纹理：{}", file_path);
        }
    }
    ...
    glm::vec2 TextureManager::getTextureSize(std::string_view file_path)
    {
        ...
        if (!texture)
        {
            spdlog::error("[TextureManager] 无法获取纹理：{}", file_path);
            return glm::vec2(0);
        }
        ...
        {
            spdlog::error("[TextureManager] 无法查询纹理尺寸：{}", file_path);
            return glm::vec2(0);
        }
        return size;
    }
